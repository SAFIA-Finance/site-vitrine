...
| **Fil d'Ariane, lien indistinguable** | Dans `.fil-ariane`, le lien « Accueil » a exactement la couleur, la graisse et l'absence de soulignement du texte qui l'entoure : rien ne le signale comme cliquable. Mesuré le 18/09/2026 sur cinq pages, dont quatre antérieures au chantier : `/particuliers/`, `/tarifs/`, `/outils/per/` et un article. C'est donc une condition **site-wide**, pas un défaut introduit par une page. Corriger sur une seule créerait une incohérence sur 165 pages, d'où ce point plutôt qu'une retouche locale. Un soulignement, ou une couleur distincte, suffirait. **Modification visible : à valider par Maxime avant exécution.** |
| **Descriptions trop longues** | Mesuré le 18/09/2026 sur le build : **19 articles de blog** ont une méta-description au-delà de 155 signes, jusqu'à 161. Ce document affirmait plus bas « 0 description au-delà de 155 » sur les articles comme sur les pages fixes ; c'est exact pour les pages fixes, faux pour les articles. Ces textes viennent du champ `Meta` des fichiers territoire, donc la correction appartient au lot 6 du chantier SEO. |
